  ...
  SECTION("MipComp::solveSeries"){
    ...
    while (std::getline(file, str)){
      if (0 < lineIndex && lineIndex <= testRunner.instanceSolvers.size() + 1){
        ...
        // lpBound
        REQUIRE(isVal(std::stod(match[2].str()), testRunner.runData[lineIndex - 1].lpBound, 1e-4));
        // disjunctiveDualBound
        REQUIRE(isVal(std::stod(match[3].str()), testRunner.runData[lineIndex - 1].disjunctiveDualBound, 1e-4));
        // lpBoundPostVpc
        REQUIRE(isVal(std::stod(match[4].str()), testRunner.runData[lineIndex - 1].lpBoundPostVpc, 1e-4));
        // rootDualBound
        ...
        // VPC generator
        REQUIRE(match[16].str() == (lineIndex == 1 ? "New" : testRunner.vpcGenerator));
        // terms
