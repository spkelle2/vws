...
std::vector < std::vector<double> > getFarkasMultipliers(
  ...
  v.resize(termSolvers.size());

  for (int term_ind = 0; term_ind < termSolvers.size(); term_ind++) {
    // For each term of the disjunction, we need to explicitly add the constraint(s)
    // defining the disjunctive term. In the form a^T x >= b
    const CoinPackedVector lhs = currCut.row();
    const double rhs = currCut.lb();
    getCertificate(v[term_ind], lhs.getNumElements(), lhs.getIndices(),
                   lhs.getElements(), rhs, termSolvers[term_ind].get(), false);
  } // loop over disjunctive terms
...
void getCertificate(
    ...
    // [in] whether to find the best basis or just use the current one in the solver
    const bool find_generating_basis) {
  ...
  // todo: if sign of any element of v changed, we're going to break getCutFromCertificate
  // todo: so go ahead and return here - the below is just safety checking anyways
...
void tightenMatrixPerturbedCut(
  ...
  // check that objective values match for terms we'll possibly use (which ensures we're using optimal bases too)
  for (int termIdx=0; termIdx < param_disj.num_terms; termIdx++){
    if (param_disj.terms[termIdx].is_feasible){
      verify(isVal(param_disj.terms[termIdx].obj, term_solvers[termIdx]->getObjValue()),
             "Disjunctive term " + std::to_string(termIdx) + " has conflicting objective values.");
    }
  }

  // get the farkas certificate for this cut relative to the current solver
  std::vector < std::vector<double> > new_v = getFarkasMultipliers(term_solvers, cut);

  // check to see if our farkas multipliers are all nonnegative
  // and that each terms multipliers have a positive value
  // (else they were nerfed for computational error and we should just move on
  for (int termIdx=0; termIdx < param_disj.num_terms; termIdx++){
    if (lessThanVal(min(new_v[termIdx]), 0) ||
        lessThanVal(0, max(new_v[termIdx]))){
      return;
    }
  }

  // if its nonnegative, we can tighten the cut
  double beta_tight = 1e50;

  // for each term that wasn't pruned
  for (int termIdx=0; termIdx < param_disj.num_terms; termIdx++){
    if (param_disj.terms[termIdx].is_feasible &&
        !lessThanVal(empiricalBound, param_disj.terms[termIdx].obj)){

      // get the tightest cut possible given the new Farkas multipliers
      std::vector<double> new_a(alpha.size(), 0.0);
      double new_b;
      getCutFromCertificate(new_a, new_b, new_v[termIdx], term_solvers[termIdx].get());

      // update the tightest known bound if we can
      if (new_b < beta_tight){
        beta_tight = new_b;
      }

      // check that new_a = alpha - the cut should be parallell
      for (int i=0; i < alpha.size(); i++){
        if (!isVal(new_a[i], alpha[i])){
          printf("Tightening matrix perturbed parametric disjunctive cut failed. new_a and alpha should be same.\n");
          printf("Cut %d, term %d, variable %d: new_a = %f, alpha = %f\n", cutIdx, termIdx, i, new_a[i], alpha[i]);
          return;
        }
      }

      // check that new_b >= beta - the cut should be tighter since we now support disjunctive term
      if (lessThanVal(new_b, beta)){
        printf("Tightening matrix perturbed parametric disjunctive cut failed. new_b should be no worse than beta.\n");
        printf("Cut %d, term %d: new_b = %f, beta_tight = %f\n", cutIdx, termIdx, new_b, beta_tight);
        return;
      }
    }
  }

  // if we didn't fail, we can tighten the cut
  cut.setLb(beta_tight);
}
